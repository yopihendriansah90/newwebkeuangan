...
class FinanceController extends Controller
{
    ...
    public function categoryStore(Request $request) { $data = $request->validate(['name' => 'required|max:80', 'type' => ['required', Rule::in(['income', 'expense'])], 'icon' => ['nullable', Rule::in(array_keys(config('category_icons')))],]); Category::create([...$data, 'icon' => $data['icon'] ?? 'more_horiz', 'user_id' => auth()->id(), 'wallet_id' => $this->wallet()->id]); return back()->with('success', 'Kategori ditambahkan.'); }
    public function categoryUpdate(Request $request, Category $category) { $data = $request->validate(['name' => 'required|max:80', 'icon' => ['nullable', Rule::in(array_keys(config('category_icons')))],]); $category->update([...$data, 'icon' => $data['icon'] ?? ($category->icon ?? 'more_horiz')]); return back()->with('success', 'Kategori diperbarui.'); }
